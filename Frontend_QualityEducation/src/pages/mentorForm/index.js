...
//             console.log(newUserId) // Assuming 'userId' is the key in the payload containing the user ID
//             // setUserId(userId);
//             setFormData({
//                 ...formData,
//                 userId: newUserId
...
//     return (
//         <div className="relative min-h-screen bg-cover bg-center" style={{ backgroundImage: ` 'images/default-background.png'})` }}>
...
import React, { useState, useEffect } from 'react';
import { jwtDecode } from 'jwt-decode'; // Assuming this is imported correctly
import { toast } from 'react-toastify';
...
const MentorForm = () => {
    const [formData, setFormData] = useState({
        ...
        graduated: false,
        language: '',
        file: null, // Store file object, not data URL
        userId: null
    });

    useEffect(() => {
        // Fetch user ID from cookie on component mount
        const cookieValue = document.cookie
            .split("; ")
            .find(row => row.startsWith("user="))
            ?.split("=")[1];
    ...
    const handleChange = (e) => {
        ...
        if (type === 'file') {
            // Handle file input separately to read file object
            const file = files[0];
            setFormData({
                ...formData,
                file: file // Store file object in formData
            });
        } else {
    ...
    const handleSubmit = async (e) => {
        ...
        // Check if file exists in formData and append it
        if (formData.file) {
        ...
        try {
            const response = await fetch('http://localhost:9090/createMentor', {
                method: 'POST',
                body: formDataToSend,
                // headers: { // If you need to set headers, add them here
            ...
            console.error('Error submitting mentor form:');
            toast.error('An error occurred while submitting the form');
        }
    };
    ...
    return (
        <div className="relative min-h-screen bg-cover bg-center" style={{ backgroundImage: `url('images/default-background.png')` }}>
            <div className="absolute inset-0 bg-black bg-opacity-60"></div>
            <div className="relative z-10 flex justify-center items-center min-h-screen">
                <form onSubmit={handleSubmit} className="bg-white bg-opacity-90 p-8 rounded-lg shadow-lg max-w-md w-full space-y-6">
                    <h2 className="text-3xl font-extrabold mb-6 text-center text-gray-900">Mentor Form</h2>
